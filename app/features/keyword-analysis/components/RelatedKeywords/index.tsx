...
import type { RelatedKeyword } from '@/features/keyword-analysis/types';
import { getCompColor } from '@/features/keyword-analysis/utils';
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <h3 className="font-semibold text-text-primary">연관 키워드</h3>
            <p className="text-xs text-text-tertiary">{keywords.length}개의 관련 키워드 발견</p>
          </div>
        </div>
        {isExpanded ? (
          ...
          <div className="flex flex-wrap gap-2">
            {keywords.map((related, idx) => (
              <button
                ...
                className="group flex items-center gap-2 px-4 py-2.5 bg-hover hover:bg-primary/10 rounded-xl transition-all hover:shadow-md"
              >
