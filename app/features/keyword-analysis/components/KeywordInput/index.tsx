    ...
    <div className="relative">
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          <div className="flex flex-wrap items-center gap-2 min-h-[52px] px-4 py-2 bg-surface border-2 border-border rounded-2xl focus-within:border-primary transition-colors">
            <Search className="w-5 h-5 text-text-tertiary flex-shrink-0" />

            {keywords.map((kw) => (
              <span
                key={kw}
                className="inline-flex items-center gap-1.5 px-3 py-1 bg-primary/10 text-primary rounded-full text-sm font-medium animate-in fade-in zoom-in duration-200"
              >
                {kw}
                <button
                  type="button"
            ...
            <input
              ...
              className="flex-1 min-w-[120px] bg-transparent outline-none text-sm text-text-primary placeholder:text-text-tertiary disabled:cursor-not-allowed"
            />
          </div>
            ...
            <div className="absolute top-full left-0 right-0 mt-2 bg-surface border border-border rounded-xl shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              {suggestions.map((suggestion, idx) => (
                <button
                  ...
                  className="w-full flex items-center gap-3 px-4 py-3 text-left text-sm hover:bg-hover transition-colors"
                >
        ...
        <button
          ...
          className="h-[52px] px-8 bg-primary text-white rounded-2xl font-semibold disabled:opacity-50 hover:opacity-90 active:scale-[0.98] transition-all flex items-center justify-center gap-2 shadow-lg shadow-primary/25"
        >
          {loading ? (
            <React.Fragment>
